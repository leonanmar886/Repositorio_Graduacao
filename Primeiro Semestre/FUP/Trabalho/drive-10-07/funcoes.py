...
def ListarDoacao(matrizDoacao):
    ...
    # for j in range(len(matrizDoacao)):
    #     data = matrizDoacao[j][6]
    #     dia = data[0:2]
    #     mes = data[3:5]
    #     ano = data[6:10]
    #     dataFormatada1 = datetime.datetime(int(ano), int(mes), int(dia))
    #     contador = 0
    #     for k in range(j,len(matrizDoacao)):
    #         linhaAux = []
    #         data2 = matrizDoacao[k][6]
    #         dia2 = data2[0:2]
    #         mes2 = data2[3:5]
    #         ano2 = data2[6:10]
    ...
    #             if contador > 0:
    #                 linhaAux.append(matrizDoacao[j])
    #                 matrizDoacao[j] = matrizDoacao[k]
    #                 matrizDoacao[k] = linhaAux[0]
    #             else:
    #                 linhaAux.append(matrizDoacao[j])
    #                 matrizDoacao[j] = matrizDoacao[k]
    #                 matrizDoacao[k] = linhaAux[0]
...
def ListarVenda(matrizVenda):
    ...
    # LerArquivo(matrizVenda,'venda.txt')
    # for i in range(len(matrizVenda)):
    #     matrizVenda[i][8] = float(matrizVenda[i][8].replace(',','.')) #percorre toda a lista, e substitui as virgulas por pontos.
    # for j in range(len(matrizVenda)):
    #     valor = matrizVenda[j][8] #fixa o valor de uma peça e compara ele com o das demais.
    #     for k in range(j,len(matrizVenda)): #o for já começa da posição j para evitar comparações indevidas
    #         linhaAux = []
    #         if valor > matrizVenda[k][8]: #se o valor da peça fixada for maior do que a do for.
    #             linhaAux.append(matrizVenda[j]) #a lista auxiliar recebe a peça mais cara.
    #             matrizVenda[j] = matrizVenda[k] #a peça mais barata vai para o lugar da peça mais cara, no caso uma linha mais acima né.
    #             matrizVenda[k] = linhaAux[0] #a peça mais cara vai para o lugar que era da peça mais barata.
    #     print(ExibirFormatado(matrizVenda[j]))

def LimparTerminal():
    os.system('cls' if os.name == 'nt' else 'clear') #codigo para limpar o terminal.

#chamar a função que o usuário desea
def ChamarFuncao(id):
    if id == 1:
