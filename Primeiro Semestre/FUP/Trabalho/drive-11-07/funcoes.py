...
def SelecionarEstilo():
    matrizPrincipal.clear() #Realiza lógica de limpar a matriz e ler os aqruivos adicionando os dados atualizados as matrizes , evitando assim
    matrizEstilos.clear()   #a duplicação de dados na matriz e garantindo dados atualizados
    LerArquivo(matrizEstilos,'estilos.txt') #Realiza a lógica de atualizar as matrizes com base nos arquivos.
    LerArquivo(matrizPrincipal,'dados.txt')
    if len(matrizEstilos) == 0: #se matriz de estilos tiver vázia, de listar.
        ListarEstilos(matrizEstilos)
    else:
        while True: #While é para se basear na confirmação de escolha do usuário.
            print('-'*47 + '\n' + 'MENU --> SELECIONAR PEÇAS POR ESTILO\n' + '-'*47)
            ListarEstilos(matrizEstilos) #Lista os estilos com base na matriz.
            
            while True:
                try: #tenta converter a entrada em inteiro, se conseguir, o laço é quebrado e o código continua, se não conseguir, o laço continua até o usuário inserir uma entrada válida.
                    estiloSelecionado = int(input('-'*47 + '\nDigite a linha do estilo que você deseja: ')) #pergunta o estilo a ser selecionado.
                    break
                except:
                    None 
            while estiloSelecionado >= len(matrizEstilos) or estiloSelecionado < 0: #se a entrada for maior do que a quantidade de estilos ou negativa, pede a entrada novamente
                try: #tenta converter a entrada em inteiro, se conseguir, o laço é quebrado e o código continua, se não conseguir, imprime uma mensagem de erro e o laço continua até o usuário inserir uma entrada válida.
            ...
            print('-'*47)
            estiloSelecionado = matrizEstilos[estiloSelecionado][0] #Acessa o estilo selecionado com base na linha da matriz.    
            for i in matrizPrincipal:
                if estiloSelecionado in i: #se o estilo estiver na linha da matriz principal, imprime a linha (a peça contendo o estilo) a imprime formatada
            ...
            if confirmacao.upper() == 'S': #se sim, o laço é quebrado e o usuário volta para o menu
                break
            if confirmacao.upper() == 'N': # se não a função roda novamente para que o usuário escolha o estilo correto.
                continue
...
def ListarEstilos(matrizDesejada): #o parametro da func sera uma matriz, no caso, a matrizEstilos
    ...
    if len(matrizDesejada) == 0: #por existir a possibilidade de não haver estilos salvos (como na primeira execução, com o arquivo limpo ainda), foi implementado um if/else para lidar com o problema
        ...
        for aux in range(len(matrizDesejada)): #a impressao a ser feita sera a de uma matriz, por conta disso, toda a ideia de lidar com matrizes sera aplicada. o for exterior vai ate o num de linhas
            for j in range(1): #o for interior vai ate 1 (por ser só um elemento, existe apenas uma coluna)
                ...
                listaAux.append(aux) #como poderão surgir sempre novos estilos, a comparação vai ser dada por indices que vão até a quantidade de estilos armazenadas
    opc = int(input('\n\nDeseja vizualizar as peças de algum desses estilos?\n1.Sim\n2.Não\n3.Vizualizar o estilo mais acessado.\n')) #apenas opções 
    if opc == 1: #caso a opção escolhida acima seja a de vizualizar as peças... A PARTIR DAQ NÃO ESTA COMENTADO
        ...
        stringAcessos = ''
        stringAcessos = stringAcessos + matrizDesejada[indice][0] + '|'
        AdcArquivo(stringAcessos,'AcessosEstilos.txt')
        listaAcessos.append(matrizDesejada[indice][0])
        if indice >= 0 and indice <= len(matrizDesejada):
            for i in range(len(matrizDesejada)):
                if indice in listaAux: 
                    cond = True
        if cond == True:
            print('O estilo desejado foi: ',matrizDesejada[indice][0],', e as peças com esse estilo AINDA no seu guarda-roupa são:')
            for i in range(len(matrizPrincipal)):  
                if matrizDesejada[indice][0] in matrizPrincipal[i]:
                    print(ExibirFormatado(matrizPrincipal[i]))
                else:
                    cond = False
            if cond == False:
                print('O estilo já passou pelo seu guarda-roupa, mas não está mais nele.')    
    elif opc == 2:
        MostrarMenu()
    elif opc == 3:
...
def EstilosEmAlta(acessos): #acessos = listaAcessos da função ListarEstilos
    ...
    listaSemRepet = []
    EmAlta = acessos[0]
    for i in range(1,len(acessos)): #começa do 1, pois o 0 é o inicializador da variavel
        maior = acessos.count(EmAlta)
        if maior < acessos.count(acessos[i]):
            EmAlta = acessos[i]
        elif maior == acessos.count(acessos[i]):
            iguais = True
            listaIguais.append(acessos[i])
        else: 
            EmAlta = EmAlta
    if iguais == True and listaIguais[0] == listaIguais[1]:
        ...
        print('-'*47 + f'\nOs estilo mais acessado recentemente foram:\n' + '-'*47)
        for i in listaIguais:
            if i not in listaSemRepet:
                listaSemRepet.append(i)
        for impress in range(len(listaSemRepet)):
            print(listaSemRepet[impress][0])
        print(f'com {acessos.count(EmAlta)} acessos cada um. Pode ser uma boa opção ver peças desse estilo!\n')
    ...
    if len(matrizOriginal) != 0:
        ...
        #cadastradas no guarda-roupa, e imprime de forma formatada todas as peças que correspondem com os valores inseridos nos filtros.
            ...
            if filtro == 1:
                ...
                for i in range(len(matrizOriginal)):
                    if matrizOriginal[i][5][0:-1] == filtroEspecifico[0:-1]: #corta a ultima letra da peça para contornar a variação de gênero das cores: ex: bermuda branca e jaqueta branca.
                        print(ExibirFormatado(matrizOriginal[i]))
            elif filtro == 7:
                for i in range(len(matrizOriginal)):
...
def ListarDoacao(matrizDoacao):
    ...
    for item in sorted(dicionarioDatas, key = dicionarioDatas.get): #um for que exibe os itens do dicionario em ordem crescente da data.
